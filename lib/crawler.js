...
class Crawler {
  ...
  async _emulate() {
    ...
    await this._page.emulate(GenuinePuppeteer.devices[this._options.device]);
  }
  ...
  _handlePageEvents() {
    this._page.on(
      'pageerror',
      (text) => void Helper.debugConsole(text.message),
    );
    this._page.on(
      'console',
      (msg) => void Helper.debugConsole(
        `${msg.type()} ${msg.text()} at ${this._options.url}`,
      ),
    );
   ...
   */
  async _waitFor() {
    if (!this._options.waitFor && !this._options.waitForTimeout
      && !this._options.waitForSelector) {
      return;
    }
    ...
    } else if (this._options.waitForTimeout) {
      // eslint-disable-next-line no-promise-executor-return
      await new Promise(r => setTimeout(r, this._options.waitForTimeout));
    } else {
      await this._page.waitForSelector(this._options.waitForSelector);
    }
  }
