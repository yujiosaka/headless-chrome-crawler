...
class Crawler {
    ...
    return this._preRequest(options)
      .then(shouldRequest => {
        ...
        return this.browser.newPage()
          .then(page => (
            this._prepareRequest(page, options)
              ...
              .then(_response => {
                ...
                return this._addJQuery(page, options)
                  ...
                  .then(result => options.onSuccess({ response, options, result }))
   ...
   * @private
   */
  _prepareRequest(page, options) {
    return Promise.all([
      this._handlePageEvents(page, options),
      this._authenticate(page, options),
      this._emulate(page, options),
      this._setExtraHeaders(page, options),
    ]);
  }
   ...
   * @private
   */
  _handlePageEvents(page, options) {
    page.on('load', () => void debugRequest(`Page loaded for ${options.url}`));
    page.on('pageerror', msg => void debugRequest(msg));
    page.on('console', msg => void debugBrowser(`Console ${msg.type} ${msg.text} for ${options.url}`));
    page.on('dialog', dialog => this._handleDialog(dialog, options));
  }
   ...
   * @private
   */
  _handleDialog(dialog, options) {
    debugBrowser(`Dialog ${dialog.type} ${dialog.message()} for ${options.url}`);
