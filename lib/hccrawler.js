...
const PurePuppeteer = require('puppeteer');
...
const devices = PurePuppeteer.devices;
...
const deviceNames = Object.keys(PurePuppeteer.devices);
...
class HCCrawler extends EventEmitter {
   ...
   * @private
   */
  async _skipRequest(options, url) {
    const allowedDomain = this._checkAllowedDomains(options);
    if (!allowedDomain) return true;
    const allowedPath = this._checkAllowedPaths(options, url);
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      if (!options.closeTargetId) {
        ...
        res.response.lastModified = '';
        res.response.headers = [];
      }
      if (!options.targetId) await crawler.close();
  ...
  async _getUserAgent(options) {
    ...
    if (PurePuppeteer.devices[options.device]) {
      return PurePuppeteer.devices[options.device].userAgent;
    }
   ...
   */
   _checkAllowedPaths(options) {
    ...
    if (options.deniedPaths && checkPathMatch(options.deniedPaths, pathname)) return false;
    if (options.allowedPaths && !checkPathMatch(options.allowedPaths, pathname)) return false;
    return true;
  }
  ...
  async _checkRequestedRedirect(options, response) {
    ...
    return await this._checkRequested(
  ...
  async _newCrawler(options, depth, previousUrl) {
    ...
    if (options.targetId) {
      ...
      page = pages.find(p => p._target._targetId === options.targetId);
    }
    ...
    console.log('Using page:', page._target._targetId, page.url());
    options.realTargetId = page._target._targetId;
