...
class HCCrawler extends EventEmitter {
  ...
  async _startRequest(options, depth) {
    ...
    const allowed = await this._checkAllowedRobots(options, depth);
    if (!allowed) {
   ...
   * @private
   */
  async _checkAllowedRobots(options, depth) {
    if (!options.obeyRobotsTxt) return true;
    const robot = await this._getRobot(options, depth);
  ...
  async _followSitemap(options, depth) {
    ...
    await Promise.resolve(map(sitemapUrls, async sitemapUrl => {
      const sitemapXml = await this._getSitemapXml(sitemapUrl, options, depth);
   ...
   * @return {!Promise<!string>}
   */
  async _getSitemapXml(sitemapUrl, options, depth) {
