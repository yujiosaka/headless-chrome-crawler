...
const { BaseCache, SessionCache, RedisCache } = require('./cache');
...
class HCCrawler {
  /**
   * Get the base cache class
   * @return {BaseCache} base cache
   * @readonly
   * @static
   */
  static get BaseCache() {
    return BaseCache;
  }

  /**
   * Get the base session class
   * @return {SessionCache} base cache
   * @readonly
   * @static
   */
  ...
  /**
   * Get the base Redis class
   * @return {RedisCache} Redis cache
   * @readonly
   * @static
   */
  ...
  close() {
    return Promise.all([
      this._browser.close(),
      this._clearCacheOnEnd().then(() => this.closeCache()),
    ]);
  }
  ...
  _checkAllowedDomains(options) {
    ...
    return _.some(options.allowedDomains, domain => _.endsWith(hostname, domain));
  }
  ...
  _checkExists(options) {
    if (!this.cache) return Promise.resolve(false);
    return this.cache.exists(options)
      .then(exists => this.cache.set(options).then(() => exists));
  }
   ...
   */
  _clearCacheOnEnd() {
    if (this._options.clearCacheOnEnd) return this.clearCache();
    return Promise.resolve();
  }
}
