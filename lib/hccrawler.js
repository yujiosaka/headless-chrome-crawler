...
class HCCrawler extends EventEmitter {
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      if (options.useSingleBrowserPage === -1) await crawler.close();
  ...
  async _newCrawler(options, depth, previousUrl) {
    var page = null;
    if (options.useSingleBrowserPage > -1) {
      var pages = await this._browser.pages();
      if (pages.length > options.useSingleBrowserPage) {
        page = pages[options.useSingleBrowserPage];
      }
    }
    if (!page) {
      page = await this._browser.newPage();
      if (options.useSingleBrowserPage > -1 && options.landingPage) {
        await page.goto(options.landingPage);
      }
    }
